...
NOTE: In quantms.io we use the UUID to identify the project and the files `{PREFIX}-{UUID}.{view}.{format}`, it is optional, but for most of the code examples we will use it.
...
| `acquisition_properties`  | Properties of the data acquisition methods   | list[string]
| `quantms_files`           | Files related to quantMS analysis            | Dict[string]
| `quantms.io_version`      | Version of the quantms.io                    | String
...
The following fields are shared amoung Peptide-based views: <<psm>>, <<feature>>, <<peptide>>.
...
| `is_decoy`         | Decoy indicator, 1 if the PSM is a decoy, 0 target             | int
...
3+^| Protein fields shared by <<feature>> <<psm>>
...
| `unique`           | Unique peptide indicator, if the peptide maps to a single protein, the value is 1, otherwise 0 | int, null
...
| `num_peaks`           | Number of peaks in the spectrum used for the peptide spectrum match | int, null
| `mz_array`            | Array of m/z values for the spectrum used for the peptide spectrum match | array[double], null
| `intensity_array`     | Array of intensity values for the spectrum used for the peptide spectrum match | array[double], null
| `rank`                | Rank of the peptide spectrum match in the search engine output | int, null
|===
...
| `spectral_count` | The number of spectra that match is given peptidoform       | intenger, null
