    ...
    def generate_psm_and_feature_file(
        ...
        for report in self.main_report_df(report_path, qvalue_threshold, mzml_info_folder, file_num):
            ...
            feature_pqwriter = self.generate_feature_file(
                report, s_data_frame, f_table, sdrf_path, feature_pqwriter, feature_output_path
            )
            et = time.time() - s
    ...
    def generate_feature_file(self, report, s_data_frame, f_table, sdrf_path, feature_pqwriter, feature_output_path):
        ...
        report = report.merge(
            (
                ...
                .rename(columns={"MSstats_BioReplicate": "BioReplicate", "MSstats_Condition": "Condition"})
                .drop(columns=["Sample"])
            ),
            on="Run",
