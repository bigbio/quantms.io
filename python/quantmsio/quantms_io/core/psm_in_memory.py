...
from quantms_io.core.feature_in_memory import FeatureInMemory,get_modifications
from quantms_io.utils.pride_utils import get_petidoform_msstats_notation,get_peptidoform_proforma_version_in_mztab,generate_scan_number
class PsmInMemory:
    ...
    def generate_psm_parquet(self,mztab_path,output_path,chunksize=1000000):
        ...
        psms = self._feature.skip_and_load_csv(mztab_path, 'PSH', sep='\t',dtype={'start':str,'end':str},chunksize=chunksize)
        ...
        for psm in psms:
            ...
            psm.loc[:,'global_qvalue'] = psm[
                    ...
                    lambda row: map_dict[
                        (row['peptidoform'], row['charge'])
                    ][0] if (row['peptidoform'], row['charge']) in map_dict else None,
                    axis=1,
                )
            ...
            psm.loc[:,'id_scores'] = (
            psm_score_name + ":" + psm["search_engine_score[1]"].astype(str) + ","
